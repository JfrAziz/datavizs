...
import { ZustandIndexedDB } from "@/components/react"
  ...
  persist(
    immer<DataState & DataAction>((set, get) => ({
      ...
      deleteColumns: (dataId, names) =>
        set((state) => {
          state.metadata[dataId].columns = state.metadata[
            dataId
          ].columns.filter((item) => !names.includes(item.name))
    ...
    {
      name: "data-store",
      storage: createJSONStorage(() => ZustandIndexedDB),
      partialize: (state) => ({
